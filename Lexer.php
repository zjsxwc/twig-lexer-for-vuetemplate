...
class Lexer extends Twig_Lexer
{
    ...
    const STATE_VAR_IN_VUE_TEMPLATE = 5;
    ...
    public function __construct(Twig_Environment $env, array $options = array())
    {
        ...
        $this->regexes = array(
            ...
            'lex_block_vue_template' => '/\s*vuetemplate\s*(?:'.preg_quote($this->options['whitespace_trim'].$this->options['tag_block'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_block'][1], '/').')/As',
            'lex_block_vue_template_close' => '/('.preg_quote($this->options['tag_block'][0].$this->options['whitespace_trim'], '/').'|'.preg_quote($this->options['tag_block'][0], '/').')\s*(?:endvuetemplate)\s*(?:'.preg_quote($this->options['whitespace_trim'].$this->options['tag_block'][1], '/').'\s*|\s*'.preg_quote($this->options['tag_block'][1], '/').')/s',
        );
    }
    ...
    public function tokenize(Twig_Source $source)
    {
        ...
        while ($this->cursor < $this->end) {
            ...
            switch ($this->state) {
                ...
                case self::STATE_VAR_IN_VUE_TEMPLATE:
                    $this->lexVarInVueTemplate();
                    break;
        ...
        switch ($this->positions[1][$this->position][0]) {
            ...
            case $this->options['tag_block'][0]:
                ...
                if (preg_match($this->regexes['lex_block_raw'], $this->code, $match, null, $this->cursor)) {
                    ...
                    $this->lexRawData();
                // {% vuetemplate %}
                } elseif (preg_match($this->regexes['lex_block_vue_template'], $this->code, $match, null, $this->cursor)) {
                    $this->moveCursor($match[0]);
                    $this->lexVueTemplate();
                // {% line \d+ %}
     ...
     * @throws Twig_Error_Syntax
     */
    private function lexVueTemplate()
    {
        ...
        if (isset($match[0])) {
            ...
            $this->pushToken(Twig_Token::VAR_START_TYPE);
            $this->pushState(self::STATE_VAR_IN_VUE_TEMPLATE);
            $this->currentVarBlockLine = $this->lineno;
        } else{
            $text = substr($this->code, $this->cursor, $endMatch[0][1] - $this->cursor);
            $this->moveCursor($text.$endMatch[0][0]);
            $this->pushToken(Twig_Token::TEXT_TYPE, $text);
        }
    }
